...
const populateDevOpsCourse = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/lms');
    console.log('MongoDB Connected');

    // Find the DevOps Stack course
    const devopsCourse = await Course.findOne({ title: /DevOps/i });
    if (!devopsCourse) {
      console.error('DevOps Stack course not found!');
      process.exit(1);
    }

    console.log(`Found course: ${devopsCourse.title}`);

    // Find the instructor
    const instructor = await User.findOne({ email: 'kwame.mensah@eduhub.com' });
    if (!instructor) {
    ...
    const existingModules = await Module.find({ course: devopsCourse._id });
    for (const module of existingModules) {
      await Lesson.deleteMany({ module: module._id });
    }
    await Module.deleteMany({ course: devopsCourse._id });
    console.log('Cleared existing modules and lessons\n');

    // Course structure with modules and lessons
    const courseStructure = [
      {
        title: 'Introduction to DevOps Stack',
        description: 'Get started with the DevOps stack and set up your development environment',
        lessons: [
          {
            title: 'What is the DevOps Stack?',
            description: 'Understanding MongoDB, Express, React, and Node.js',
            type: 'recorded',
            videoUrl: 'https://www.youtube.com/embed/7CqJlxBYj-M',
            duration: 25,
            content: `# What is the DevOps Stack?

The DevOps stack is a popular JavaScript stack for building modern web applications. It consists of:
...
- High performance and scalability`,
            objectives: [
              'Understand what the DevOps stack is',
              'Learn the role of each technology',
              'Recognize the benefits of using DevOps'
            ],
            isFree: true
          },
          {
...
\`\`\``,
      ...
      {
        title: 'Final Project: Full-Stack DevOps Application',
        description: 'Build a complete DevOps stack application from scratch',
        lessons: [
          {
...
- Task statistics dashboard`,
          ...
          },
          {
            ...
            description: 'Deploy your DevOps application to production',
            type: 'reading',
            duration: 45,
...
5. Plan improvements`,
    ...
    const allLessons = await Lesson.find({ course: devopsCourse._id });
    const totalDuration = allLessons.reduce((sum, lesson) => sum + lesson.duration, 0);

    await Course.findByIdAndUpdate(devopsCourse._id, {
      duration: totalDuration
    });

    console.log(`\n✅ Successfully populated DevOps course!`);
